...
class MosaicApp:
    def __init__(self, root):
        ...
        self.mode = "manual_fanza"  # "auto", "manual_fanza", "manual_custom"
    ...
    def setup_ui(self):
        ...
        self.mosaic_size_slider = ttk.Scale(size_frame, from_=5, to=50, orient=tk.HORIZONTAL)
        self.mosaic_size_slider.set(20)
        self.mosaic_size_slider.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=5)
        ...
        # パラメータ表示用のラベル
        self.param_labels = {
            'mode': ttk.Label(param_frame, text="モード: 自動"),
    ...
    def update_parameter_display(self):
        ...
        if self.mode == "manual_fanza":
            ...
            # カスタムモードの場合はスライダーの値を表示
            self.param_labels['mosaic_size'].config(
                text=f"モザイクサイズ: {self.mosaic_size_slider.get()}"
            )
    ...
    def toggle_mode(self):
        ...
        if self.mode == "auto":
            # 自動モードは無効化
            messagebox.showwarning("自動モード無効", "自動モード（YOLO検出）は現在ご利用いただけません。手動(FANZA)モードに切り替えます。")
            self.mode = "manual_fanza"
            self.mode_button.config(text="手動(FANZA)")
            self.mosaic_size_slider.config(state='disabled')  # スライダーを無効化
        elif self.mode == "manual_fanza":
            self.mode = "manual_custom"
            self.mode_button.config(text="手動(カスタム)")
            self.mosaic_size_slider.config(state='normal')  # スライダーを有効化
        else:
            # 自動モードは無効化
            messagebox.showwarning("自動モード無効", "自動モード（YOLO検出）は現在ご利用いただけません。手動(FANZA)モードに切り替えます。")
            self.mode = "manual_fanza"
    ...
    def on_canvas_click(self, event):
        if self.current_image is None or self.mode == "auto":
            return
        ...
        # モザイク処理を実行
        custom_mosaic_size = self.mosaic_size_slider.get() if self.mode == "manual_custom" else None
        self.current_image = self.processor.process_click(
            self.current_image, img_x, img_y, self.mode, custom_mosaic_size
        )
        self.processed_image = self.current_image.copy()
