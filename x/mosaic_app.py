...
class MosaicApp:
    ...
    def update_parameter_display(self):
        ...
        if self.mode == "manual_fanza":
            # FANZA仕様のモザイクサイズを計算
            mosaic_size = self.processor.calculate_fanza_mosaic_size(self.current_image.shape)
    ...
    def on_canvas_drag(self, event):
        ...
        # 楕円の中心と半径を計算
        center_x = (self.canvas_drag_start[0] + event.x) / 2
        center_y = (self.canvas_drag_start[1] + event.y) / 2
        radius_x = abs(event.x - self.canvas_drag_start[0]) / 2
        radius_y = abs(event.y - self.canvas_drag_start[1]) / 2
        
        # 楕円を描画
        self.drag_rect = self.canvas.create_oval(
    ...
    def on_canvas_release(self, event):
        ...
        # 楕円の中心と半径を計算（画像座標系）
        center_x = (self.drag_start[0] + self.drag_end[0]) / 2
        center_y = (self.drag_start[1] + self.drag_end[1]) / 2
        radius_x = abs(self.drag_end[0] - self.drag_start[0]) / 2
        radius_y = abs(self.drag_end[1] - self.drag_start[1]) / 2
        
        # 楕円の外接矩形を計算
        x1 = int(center_x - radius_x)
        y1 = int(center_y - radius_y)
        x2 = int(center_x + radius_x)
        y2 = int(center_y + radius_y)
        
        # ドラッグ範囲が0,0の場合は処理をスキップ
        if x1 == x2 and y1 == y2:
            # ドラッグ状態をリセット
            self.is_dragging = False
            self.drag_start = None
        ...
        cv2.ellipse(mask, (int(center_x), int(center_y)), 
                   (int(radius_x), int(radius_y)), 0, 0, 360, 255, -1)
        
        # モザイクを適用（原点基準）
        temp_image = self.processor.apply_mosaic_with_origin(
            temp_image, x1, y1, x2, y2, mosaic_size
        )
        
        # 楕円の外側を現在の画像に戻す
        mask_3channel = cv2.merge([mask, mask, mask])
        self.current_image = np.where(mask_3channel == 255, 
                                    temp_image, 
                                    self.current_image)
    ...
    def quick_save_image(self):
        """ラジオボタンで選択した形式で即保存（保存中インジケータ付き、元画像名ベースの連番）"""
        if self.current_image is None:
            messagebox.showwarning("警告", "処理済み画像がありません。")
            return
        ext = self.save_format_var.get()
        folder = os.path.dirname(self.current_image_path) if self.current_image_path else os.getcwd()
        # 元画像名ベース
        if self.current_image_path:
            base = os.path.splitext(os.path.basename(self.current_image_path))[0]
        else:
        ...
        try:
            if ext == "png":
                ...
                # 4チャンネル（BGRAなど）の場合は3チャンネル（BGR）に変換
                if len(img.shape) == 3 and img.shape[2] == 4:
                    img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
                # 型がuint8でなければ変換
                if img.dtype != np.uint8:
                    img = img.astype(np.uint8)
                # 2次元（グレースケール）の場合はBGRに変換
                if len(img.shape) == 2:
                    img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
                cv2.imwrite(candidate_path, img)
            messagebox.showinfo("成功", f"画像を保存しました: {candidate_path}")
            # フォルダ内容をリロード
            self.reload_folder_contents()
        except Exception as e:
