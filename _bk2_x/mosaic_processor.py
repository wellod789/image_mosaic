...
Contact: alexia.douglas789@vellod-ec.com"""
    ...
    def process_masked_area(self, image, mask_coords, drag_coords, mode, custom_mosaic_size=None, multiplier=1):
        ...
        # 実際に処理する範囲を計算（ドラッグ範囲と処理範囲の重複部分）
        proc_x1 = max(drag_x1, mask_x1)
        proc_y1 = max(drag_y1, mask_y1)
        proc_x2 = min(drag_x2, mask_x2)
        proc_y2 = min(drag_y2, mask_y2)
        ...
        # モザイクサイズの決定
        if mode == "manual_fanza":
            base_mosaic_size = self.calculate_fanza_mosaic_size(img.shape)
        else:
            base_mosaic_size = int(custom_mosaic_size)
        
        # 倍率を適用
        mosaic_size = base_mosaic_size * multiplier
        if mosaic_size == 0: mosaic_size = 1 # 0除算を避ける
        ...
        # ループは画像全体の絶対座標を基準に行う
        for y in range(proc_y1, proc_y2, click_interval):
            for x in range(proc_x1, proc_x2, click_interval):
                
                # クリック位置をモザイクサイズで割り切れるように調整（画像の原点が基準）
                adjusted_x = (x // mosaic_size) * mosaic_size
                adjusted_y = (y // mosaic_size) * mosaic_size
                
                # モザイクを適用する領域の座標を計算
                mx1 = max(0, adjusted_x - area_size)
                my1 = max(0, adjusted_y - area_size)
                mx2 = min(img_width, adjusted_x + area_size)
                my2 = min(img_height, adjusted_y + area_size)
                
                # 実際にモザイクをかける範囲を、処理範囲（proc）内にクリッピングする
                # これにより、モザイクが処理範囲の外にはみ出さないようにする
                final_mx1 = max(mx1, proc_x1)
                final_my1 = max(my1, proc_y1)
                final_mx2 = min(mx2, proc_x2)
                final_my2 = min(my2, proc_y2)

                if final_mx1 < final_mx2 and final_my1 < final_my2:
                    img = self.apply_mosaic(img, final_mx1, final_my1, final_mx2, final_my2, mosaic_size)
