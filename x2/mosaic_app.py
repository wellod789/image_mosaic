...
class MosaicApp:
    ...
    def setup_ui(self):
        ...
        radio_frame.grid(row=0, column=6, padx=4, sticky=tk.EW)
    ...
    def update_parameter_display(self):
        ...
        if self.mode == "manual_fanza":
            # FANZA仕様のモザイクサイズを計算
            mosaic_size = self.image_processor.calculate_fanza_mosaic_size(self.current_image.shape)
            self.param_labels['mosaic_size'].config(
                text=f"モザイクサイズ: {mosaic_size} (FANZA仕様)"
            )
        else:
            # カスタムモードの場合は入力フィールドの値を表示
            self.param_labels['mosaic_size'].config(
                text=f"モザイクサイズ: {self.mosaic_size_var.get()}"
            )
    ...
    def select_image(self):
        ...
        if file_path:
            try:
                # 画像を読み込む
                self.original_image = self.file_manager.load_image(file_path)
                ...
                # プレビューマネージャーの更新
                self.preview_manager.reload_folder_contents(file_path)
                
                # 履歴のクリアと初期状態の追加
                self.history_manager.clear_history()
                self.history_manager.add_to_history(self.current_image)
    ...
    def load_folder_image(self, index):
        ...
        image = self.preview_manager.load_folder_image(index, self.file_manager.load_image)
    ...
    def on_canvas_click(self, event):
        if self.current_image is None or self.preview_manager.preview_mode:
            return
    ...
    def on_canvas_drag(self, event):
        if not self.is_dragging or self.current_image is None or self.preview_manager.preview_mode:
            return
    ...
    def on_canvas_release(self, event):
        ...
        if self.mode == "manual_fanza":
            ...
            mosaic_size = int(self.mosaic_size_var.get())
        ...
        for y in range(y1, y2, click_interval):
            for x in range(x1, x2, click_interval):
                if 0 <= x < img_width and 0 <= y < img_height:
                    temp_image = self.image_processor.process_click(
                        temp_image,
                        x, y,
                        self.mode,
                        mosaic_size
    ...
    def reload_folder_contents(self):
        ...
        if self.current_image_path:
            ...
            # プレビュー情報を更新
            if self.preview_manager.preview_mode:
    ...
    def save_image(self):
        ...
        if file_path:
            # 画像を保存
            success = self.image_processor.save_with_metadata(
                self.current_image,
                file_path
            )
    ...
    def toggle_preview_mode(self):
        """プレビューモードの切り替え"""
        if not self.preview_manager.preview_mode:
            # プレビューモードを開始
            self.preview_manager.preview_mode = True
            self.preview_button.config(text="プレビュー終了")
            
            # 現在の画像をプレビューリストに追加
            if self.current_image is not None:
                self.preview_manager.preview_images = [self.current_image.copy()]
                self.preview_manager.current_preview_index = 0
    ...
    def exit_preview_mode(self, event=None):
        """プレビューモードを終了"""
        if self.preview_manager.preview_mode:
            self.preview_manager.preview_mode = False
    ...
    def display_preview_image(self):
        """プレビュー用の画像を表示"""
        if not self.preview_manager.preview_images:
            return
            
        # 現在のプレビュー画像を表示
        self.display_image(self.preview_manager.preview_images[self.preview_manager.current_preview_index])
    ...
    def update_preview_info(self):
        ...
        # プレビュー情報を取得
        preview_info = self.preview_manager.get_preview_info()
        if preview_info:
            info_text = f"プレビュー: {preview_info['current_index']}/{preview_info['total_images']}"
    ...
    def previous_image(self, event=None):
        ...
        if self.preview_manager.can_move_previous():
            if self.load_folder_image(self.preview_manager.current_folder_index - 1):
                self.update_preview_info()
    ...
    def next_image(self, event=None):
        ...
        if self.preview_manager.can_move_next():
            if self.load_folder_image(self.preview_manager.current_folder_index + 1):
                self.update_preview_info()
    ...
    def quick_save_image(self):
        ...
        # 保存先フォルダの設定
        base_folder = os.path.dirname(self.file_manager.current_image_path) if self.file_manager.current_image_path else os.getcwd()
        ...
        if self.file_manager.current_image_path:
            base = os.path.splitext(os.path.basename(self.file_manager.current_image_path))[0]
        else:
            base = "output"
        
        filename = self.file_manager.get_next_filename(completed_folder, base, ext)
        save_path = os.path.join(completed_folder, filename)
        ...
        def save_task():
            try:
                # モザイク処理済み画像の保存
                if self.file_manager.save_image(self.current_image, save_path, ext):
                    # オリジナル画像の移動
                    if self.file_manager.current_image_path:
                        self.file_manager.move_original_file(
                            self.file_manager.current_image_path,
                            original_folder
                        )

                    # フォルダ内容をリロード
                    self.root.after(0, self.preview_manager.reload_folder_contents, self.file_manager.current_image_path)

                    # プレビューモードの場合、次の画像に移動
                    if self.preview_manager.preview_mode and self.preview_manager.can_move_next():
                        self.root.after(0, lambda: self.load_folder_image(self.preview_manager.current_folder_index + 1))
